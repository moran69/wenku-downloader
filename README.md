...
- **多人使用**：用户账号体系（管理员/普通用户），每人独立的任务记录和下载空间，管理员网页上增删用户、重置密码
...
- **Cookie 管理**：管理员在网页上直接粘贴更新 Cookie（全体用户共享会员凭证），支持联网检测登录态
- **安全**：密码 PBKDF2 哈希存储、Session 持久化、文件按用户隔离、并发下载限制、防路径穿越
...
访问 `http://<服务器IP>:18900`。首次启动会自动创建管理员账号 **admin**（密码取自 `password.txt`，没有则随机生成并打印在日志里）。

### 多用户

登录后普通用户只能看到自己的任务和文件；管理员额外拥有：

- **用户管理**：网页上直接添加用户、重置密码、删除用户（连同其文件和记录）
- **Cookie 管理**：更新全体用户共享的会员 Cookie
- **全局视图**：在「我的 / 全部用户」之间切换，查看所有人的任务与文件

把链接发给家人朋友，各自注册使用即可（由管理员创建账号）。每人同时最多排队 10 个任务，全局并发下载数默认 2（可用 `WENKU_MAX_CONCURRENT` 调整）。
